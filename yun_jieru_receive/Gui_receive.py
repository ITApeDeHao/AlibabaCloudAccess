...
import base64
import json
import iot_data_from_yun_receive as idfy
import Link_ali_receive as lar
...
class Gui_get(object):
    def __init__(self):
        self.window = tk.Tk()
        self.window.title("设备端")
    ...
    def get_text(self):
        try:
            with open('./receive_message/1.txt','r') as f:
                content = f.read()
            tk.Label(self.frame,text=content,width=50,height=15,justify='left').place(relx=0.47,rely=0.15)
        except:
