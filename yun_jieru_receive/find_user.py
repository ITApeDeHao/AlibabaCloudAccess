...
class Enroll_database(object):
	...
	def __init__(self,admin_name,admin_pwd,admin_window):
		...
		self.db = pm.connect(host='localhost', user='username', passwd='paswd', port=3306)
		# 链接数据库游标
				...
				if self.idword != None:
					...
					if self.admin_name == self.idword[1]:
						...
						if self.admin_pwd == self.idword[2]:
							# 提示框，登陆成功
							tk.messagebox.showinfo(title='成功',message='登陆成功,可以传输数据!')
							# 登陆界面自动关闭，打开爬取界面
							...
							# 密码错误后，提示窗口
							tk.messagebox.showerror(message='密码错误，您无权上传。')
			# 用户名不在数据库中，提示框
			except:
